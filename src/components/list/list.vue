<template>
    <div id="listbox">
        ...
        <div class="love">
            ...
            <ul class="content">
                <li v-for="(item,n) in category">
                    <router-link :to="'/listofid?id='+item.id">
                        <div class="con">
                            <h3>
                                <span>{{item.name}}</span>系列
                            </h3>
                            <p>{{item.ename}}</p>
                        </div>
                        <div class="tu">
                            <img :src="item.coverimg[0].url" alt="">
                            <div>
                                <p>MORE</p>
                            </div>
                        </div>
...
<script>
    export default {
        ...
        created(){
            this.$http.get('/api/index/list').then(response => {
                response.body.forEach(val=>{
                    val.thumb = JSON.parse(val.thumb);
                });
                this.list=response.body;
            });
            this.$http.get('/api/index/list/cate?len=0').then(response => {
                response.body.forEach(val=>{
        ...
        methods:{
            select(){
                if (this.flag){
                    this.$http.get('/api/index/list/cate?len=3').then(response => {
                        response.body.forEach(val=>{
                            val.coverimg = JSON.parse(val.coverimg);
                        });
                        this.category=response.body;
